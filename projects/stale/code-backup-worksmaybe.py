...
def set_speed(left_speed: float, right_speed: float, ramp_time=0.5):
    ...
    steps = 5  # Reduced steps for faster response
    ...
    # Explicitly set final speeds
    PWM_L.duty_cycle = int(left_speed * 65535)
    PWM_R.duty_cycle = int(right_speed * 65535)

def move(forward=True):
    """ Moves the robot at the selected speed in the given direction for a fixed duration. """
